...
public sealed class ModButton_Installed : ModButton
{
    ...
    public Color Color
    {
        get
        {
            ...
            if (SCPModManager.UserData[Selected].Color != Color.white)
            {
                return SCPModManager.UserData[Selected].Color;
            }

            // then button colour
            if (SCPModManager.UserData[this].Color != Color.white)
            {
                return SCPModManager.UserData[this].Color;
            }
        ...
        set => SCPModManager.UserData[this].Color = value;
    }
    ...
    public override bool IsSCPModManager => Selected != null && Selected.SamePackageId("scplegion.scpmodmanager");
    ...
    public static ModButton_Installed For(ModMetaData mod)
    {
        var button = ModButtonManager.AllButtons.OfType<ModButton_Installed>()
            .FirstOrDefault(mb =>
                mb.Name == mod.Name || SCPModManager.Settings.TrimTags && mb.TrimmedName == TrimModName(mod.Name));
        if (button == null)
        {
        ...
        if (ButtonIcon(ref iconRect, Palette, I18n.ChangeColour))
        {
            ...
            options.Add(new FloatMenuOption(I18n.ChangeModColour(Name), () => Find.WindowStack.Add(
                new Dialog_ColourPicker(Color, color =>
                    SCPModManager.UserData[Selected].Color = color
                ))));
            options.Add(new FloatMenuOption(I18n.ChangeButtonColour(Name), () => Find.WindowStack.Add(
                new Dialog_ColourPicker(Color, color =>
                    SCPModManager.UserData[this].Color = color
                ))));
            FloatMenu(options);
        }
    ...
    private void DoModActionFloatMenu()
    {
        ...
        options.Add(new FloatMenuOption(I18n.ChangeColour, () =>
        {
            ...
            options2.Add(new FloatMenuOption(I18n.ChangeButtonColour(Name), () => Find.WindowStack.Add(
                new Dialog_ColourPicker(Color,
                    color => SCPModManager.UserData[this].Color = color
                ))));
            FloatMenu(options2);
        }));
